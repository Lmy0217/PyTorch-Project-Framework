...
class LeNet(models.BaseModel):
    ...
    def train(self, epoch_info, sample_dict):
        ...
        loss = self.criterion(fake_validity, real_target.reshape(real_target.shape[0]))
        loss.backward()
        self.optimizer.step()

        accuracy = models.functional.algorithm.correct(fake_validity, real_target, return_label=False) / real_target.shape[0]
