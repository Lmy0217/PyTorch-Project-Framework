...
class Generator(nn.Module):
    ...
    def __init__(self, in_dims, out_shape):
        ...
        self.block1 = self._block(in_dims, 128, normalize=False)
    ...
    def forward(self, x):
        ...
        x = self.fc(x)
        x = self.tanh(x)
        x = x.view(x.shape[0], *self.out_shape)
        return x
...
class WGAN_GP(models.BaseModel):
    ...
    def __init__(self, cfg, data_cfg, run, **kwargs):
        ...
        self.G = Generator(
            in_dims=self.data_cfg.source.elements,
            out_shape=(self.data_cfg.target.time, self.data_cfg.target.height, self.data_cfg.target.width)
        ).to(self.device)
        self.D = Discriminator(in_dims=self.data_cfg.target.elements).to(self.device)
    ...
    def check_cfg(data_cfg, cfg):
        return data_cfg.source.elements == data_cfg.target.elements
    ...
    def _compute_gradient_penalty(self, real_target, fake_target):
        ...
        alpha = torch.rand((real_target.shape[0], *([1] * (real_target.ndim - 1)))).to(self.device)
        ...
        gradients = torch.autograd.grad(
            outputs=d_interpolates, inputs=interpolates, grad_outputs=fake_label,
            create_graph=True, retain_graph=True, only_inputs=True)[0]
        gradients = gradients.view(gradients.size(0), -1)
        gradient_penalty = ((gradients.norm(2, dim=1) - 1) ** 2).mean()
        return gradient_penalty

    def _train_G(self, real_source):
    ...
    def train(self, epoch_info, sample_dict):
        ...
        loss_d = self._train_D(real_source, real_target)
        loss_d.backward()
        ...
        if epoch_info['batch_idx'] % self.cfg.n_critic == 0:
            loss_g = self._train_G(real_source)
            loss_g.backward()
